...
**建模前先和业务方核对（不急着打开 DocType）：**

1. 物料编号唯一代表什么？供应商换型号时，沿用原物料还是建新物料？
2. BOM 的版本由什么事件改变？旧版是否仍需查询？谁有权批准 ECO？
3. 用一条正常变更和一条例外变更检验答案；若答案互相矛盾，先修订业务规则，再设计字段和流程。

**建模思路：** 先把确认后的业务对象、字段、关联和权限写成一张小表，再判断哪些由 DocType 元数据和框架标准能力提供，哪些确实需要自定义代码。不要把“为每个对象分别写 CRUD 页面与接口”当成第一步。
...
**解释检查：** 说出一个可由元数据直接描述的需求，以及一个必须先由业务方澄清、不能靠添加字段解决的分歧。系统使确认的规则更易执行，不代表管理规则已自动变好。
