...
**系统学习入口：** [Frappe 系统教材：从业务模型到可交付应用](./SYSTEM_TEXTBOOK.md)。先看全局地图和核心机制，再按下方实验逐层练习；已有教学文件仍可作为分专题参考。
...
flowchart LR
    A["业务理解"] --> B["DocType 建模"]
    B --> D["Controller 与 Client Script"]
    D --> C["权限与 Workflow"]
    C --> E["ECO 升版逻辑"]
    E --> F["Query Report"]
    F --> G["REST API 集成"]
...
以下时间是课堂参考时间，不含课前导学；实验三至四、五至六的时间按[讲师手册](./TEACHER_GUIDE.md)的合计课时平均分配。操作与验收细节见[实训部分](./PRACTICAL_LABS.md)，完成一行后可用最后一列自查是否真正理解。

| 实验 / 能力 | 开始前应会 | 参考时间 | 动手产出 | 学完能独立解释的问题 |
| --- | --- | --- | --- | --- |
| 一：业务实体建模 | 读懂项目规格中的物料、BOM、ECO；认识 Field | 150 分钟 | 创建三个主 DocType、BOM 子表并录入示例数据 | 为什么成品和子项要用 Link，而 BOM 明细要用 Table？ |
| 二：业务逻辑与自动化 | 完成实验一；看懂 Python 条件和 JavaScript 事件 | 120 分钟 | 为 BOM 加保存校验与表单数量合计反馈 | 为什么页面实时提示不能代替保存时的后端校验？ |
| 三：权限与流程 | 完成实验一；认识角色和单据状态 | 约 90 分钟 | 配置角色、BOM 权限和 ECO Workflow，并用不同账号验证 | 为什么角色权限与审批动作要分别检查？ |
| 四：ECO 驱动升版 | 完成实验二、三；理解关联单据与提交事件 | 约 90 分钟 | 演示 ECO 发布触发关联 BOM 的版本更新 | ECO、BOM 和目标版本如何构成一条可说明的变更链？ |
| 五：Query Report | 完成实验一；了解 SQL 的表、关联和过滤 | 约 75 分钟 | 生成可按成品物料筛选的 BOM 构成报表 | 报表如何通过主子表关联回答“产品由什么组成”？ |
| 六：REST API | 完成实验一、三；理解请求、身份与权限 | 约 75 分钟 | 用 Token 读取并创建测试物料 | 认证成功与有权操作分别意味着什么？ |
| 七：测试与交付 | 完成前六个实验；知道关键业务规则和配置 | 120 分钟（含结课演示准备） | 验证核心校验、导出 fixtures，并对照交付清单演示 | 为什么界面上做通一次不等于换个环境也能交付？ |

建议每章先尝试回答最后一列，再对照[参考代码](./CODE_SNIPPETS.md)和[最终交付清单](./FINAL_DELIVERY_CHECKLIST.md)复盘；回答不出来时，回到对应实验的操作与验收步骤。
